...
function reverseString(str) {
  const result = str.split('').reverse().toString();
...
function orderAlphabetically(str) {
  const result = str.split('').sort().toString();
  return result.replaceAll(',', '');
}
...
function isPalindrome(str) {
  ...
  if (str.endsWith('?') || str.endsWith('!')) {
    newStr = newStr.slice(0, str.length - 2);
  }
  if (str.includes(' ')) {
...
function findLongestWord(sentence) {
  const resultSplit = sentence.split(' ');
  const resultSort = resultSplit.sort((a, b) => b.length - a.length);
  const result = resultSort.slice(0, 1);
  return result;
}
...
function reverseWords(str) {
  ...
  let words = result.map((word) => word.split('').reverse().join(''));
...
function extractNameFromTemplate(value) {
  return value.slice(value.indexOf(',') + 2, value.lastIndexOf(0));
}
...
function encodeToRot13(str) {
  const abra = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const cadabra = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      ...
      for (let n = 0; n < abra.length; n += 1) {
        if (str[i] === abra[n]) {
          abraCadabra += cadabra[n];
        }
      }
    } else {
      abraCadabra += str[i];
